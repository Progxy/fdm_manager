...
class AddManager extends StatefulWidget {
  static const String routeName = "/addManager";
  AddManager({this.app});
...
class _AddManagerState extends State<AddManager> {
  ...
  addManager(String email, String name) async {
    final FirebaseAuth _firebaseAuth =
        FirebaseAuth.instanceFor(app: widget.app);
    final String password = passwordGenerator();
    final String result =
        await AuthenticationService(_firebaseAuth).signUp(email, password);
    if (result == "Operazione effetuata con successo!") {
      addDataToDatabase(email, name);
      final String text =
          "Salve,\n\nè stato aggiunto un account manager a suo nome con cui potrà accedere all'applicazione FdmManager.\n\nEcco di seguito le sue credenziali di accesso :\n\nEmail : $email ,\n\nPassword : $password.\n\nCordiali Saluti, Ermes Fdm Express.";
              ...
              child: TextButton(
                ...
                onPressed: () async {
                  if (_formKey.currentState.validate()) {
                    ...
                    final String result = await addManager(email, name);
                    await dialog.hide();
                    if (Platform.isIOS) {
                      showCupertinoDialog(
                        context: context,
